...
class Model {
     ...
     * @return mixed
     */
    public function mSelect($options=array()) {
        $pk   =  $this->getPk();
        if(is_string($options) || is_numeric($options)) {
            // 根据主键查询
            if(strpos($options,',')) {
        ...
        if ($_POST['page']) {
            ...
            $page = $page > 0 ? $page : 1;
            $rows = $_POST['rows'] ? $_POST['rows'] : 20;
            $options['limit'] = ($page - 1) * $rows .','.$rows;
        }
        ...
        return array('total'=>$this->where($options['where'])->count(), 'rows'=>$resultSet);
    }
}